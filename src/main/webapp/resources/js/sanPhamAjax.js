$(document).ready(function() {
	...
	function ajaxGet(page){
		...
		$.ajax({
			...
			url: "http://localhost:8080/luxuryshop/api/san-pham/all" + '?page=' + page,
			success: function(result){
				$.each(result.content, function(i, sanPham){
					var sanPhamRow = '<tr>' +
					                  '<td>' + '<img src="/luxuryshop/img/'+sanPham.id+'.png" class="img-responsive" style="height: 50px; width: 50px" />'+'</td>' +
					...
					var checkTenDanhMuc = (sanPham.danhMuc.tenDanhMuc.toLowerCase()).indexOf("Phòng".toLowerCase());
					sanPhamRow += ( checkTenDanhMuc != -1)?'<button class="btn btn-primary btnCapNhatSanPham" >Cập nhật</button>':'<button class="btn btn-primary btnCapNhatOther" >Cập nhật</button>';
	...
	$('#danhMucDropdown').mouseup(function() {
		...
		if (open) {
			...
			if ((label.toLowerCase()).indexOf("Phòng".toLowerCase()) != -1) {
				$('.sanPhamModal').modal('show');
				$("#idDanhMucSanPham").val($(this).val());
				$('#sanPhamForm').removeClass().addClass("addsanPhamForm");
				$('#sanPhamForm #btnSubmit').removeClass().addClass("btn btn-primary btnSavesanPhamForm");
			} else {
	...
	$('.sanPhamModal, .otherModal').on('hidden.bs.modal', function(e) {
		e.preventDefault();
		$("#idDanhMucSanPham, #idDanhMucKhac").val("");
		$("#idSanPham, #idSanPhamKhac").val("");
			
	    $('#sanPhamForm').removeClass().addClass("sanPhamForm");
		$('#sanPhamForm #btnSubmit').removeClass().addClass("btn btn-primary");
		$('#sanPhamForm').trigger("reset");
	...
	// btn Save Form Event
    $(document).on('click', '.btnSavesanPhamForm', function (event) {
    	event.preventDefault();
		ajaxPostSanPham();
		resetData();
    });
 
    function ajaxPostSanPham() {
    	// PREPATEE DATA
    	 var form = $('.addsanPhamForm')[0];   	 
    	 ...
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/luxuryshop/api/san-pham/save",
    ...
    // vs danh mục
    $(document).on("click",".btnCapNhatSanPham", function(event){
		event.preventDefault();
		var sanPhamId = $(this).parent().prev().children().val();	
		$('#sanPhamForm').removeClass().addClass("updateSanPhamForm");
		$('#sanPhamForm #btnSubmit').removeClass().addClass("btn btn-primary btnUpdateSanPhamForm");
	
		var href = "http://localhost:8080/luxuryshop/api/san-pham/"+sanPhamId;
		$.get(href, function(sanPham) {
			populate('.updateSanPhamForm', sanPham);
			$("#idDanhMucSanPham").val(sanPham.danhMuc.id);
			var hangSXId = sanPham.hangSanXuat.id;
			$("#nhaSXId").val(hangSXId);	
		});
		
		removeElementsByClass("error");		
		$('.updateSanPhamForm .sanPhamModal').modal();
	});
    
	// btn update form Event
    $(document).on('click', '.btnUpdateSanPhamForm', function (event) {
    	event.preventDefault();
		ajaxPutSanPham();
		resetData();
    });
 
    function ajaxPutSanPham() {
    	
   	 var form = $('.updateSanPhamForm')[0];
	 var data = new FormData(form);
	 console.log(data);
    ...
    $(document).on("click",".btnXoaSanPham", function() {
		...
		if(confirmation){
		  $.ajax({
			  async:false,
			  type : "DELETE",
			  url : "http://localhost:8080/luxuryshop/api/san-pham/delete/" + sanPhamId,
    ...
    $(document).on("click",".btnChiTiet", function() {
    	...
    	var href = "http://localhost:8080/luxuryshop/api/san-pham/"+sanPhamId;
		$.get(href, function(sanPham) {
			$('.hinhAnh').attr("src", "/luxuryshop/img/"+sanPham.id+".png");
			...
			if(checkTenDanhMuc != -1){
			  $('.cpu').html("<span style='font-weight: bold'>Chất liệu: </span>"+ sanPham.chatLieu);
			  $('.ram').html("<span style='font-weight: bold'>Kiểu dáng: </span>"+ sanPham.kieuDang);
			  $('.thietKe').html("<span style='font-weight: bold'>Kích thước: </span>"+ sanPham.kichThuoc);
			  $('.dungLuongPin').html("<span style='font-weight: bold'>Phong cách: </span>"+ sanPham.phongCach);
			  $('.heDieuHanh').html("<span style='font-weight: bold'>Đơn vị tính: </span>"+ sanPham.dvt);
			  $('.manHinh').html("<span style='font-weight: bold'>Hoàn thiện: </span>"+ sanPham.hoanThien);
			}
